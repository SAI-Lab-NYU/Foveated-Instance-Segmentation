...
class SegmentationModuleBase(nn.Module):
    ...
    def pixel_acc(self, pred_all, label_all):
        ...
        acc_accu = 0.
        for i in range(bs):
            pred, label= pred_all[i:, :, :], label_all[i:, :, :]
            _, preds = torch.max(pred, dim=1)
            valid = (label > 0).long()
            valid1 = (preds > 0).long()
            acc_sum = torch.sum(valid * (preds == label).long())   # this is the intersectory pixels
...
class DeformSegmentationModule(SegmentationModuleBase):
    def __init__(self, net_encoder, net_decoder, net_saliency, net_compress, crit, cfg, deep_sup_scale=None):
        ...
        self.crit = FocalLoss(gamma=15.0)
    ...
    def forward(self, feed_dict, *, writer=None, segSize=None, F_Xlr_acc_map=False, count=None, epoch=None, feed_dict_info=None, feed_batch_count=None):
        ...
        hidx_B = (torch.clamp(x_Bx2[:, 0], max=0.99) * (HS - 1))
        widx_B = (torch.clamp(x_Bx2[:, 1], max=0.99) * (WS - 1))
        ...
        if segSize is None:
            ...
            if self.deep_sup_scale is not None: # use deep supervision technique
                ...
                #print(self.decoder)
                #print(pred.shape)  # torch.Size([1, 2, 64, 128])
                #exit()
            ...
            if self.cfg.MODEL.loss_at_high_res:
                ...
                #print(pred.shape)   # torch.Size([1, 20, 64, 128]) 
                #print(feed_dict['seg_label'])  # torch.Size([1, 64, 128])

                label1 = feed_dict['seg_label']  # torch.Size([1, 64, 128])
                #print('label1:', label1)
                #print('label11111:', feed_dict['cls_label'])
                ground_truth = 19*(1-label1).cuda() + label1 * feed_dict['cls_label'][:, :, None].repeat(1, 64, 128).cuda()
            ...
            if self.cfg.MODEL.loss_at_high_res:
                ...
                ##################################### enter here!!!!############################
                acc = self.pixel_acc(pred, feed_dict['seg_label']* feed_dict['cls_label'][:, :, None].repeat(1, 64, 128))
            if self.cfg.TRAIN.deform_joint_loss:
