...
class ResNet(nn.Module):
    def __init__(self, block, inplanes = 960, num_classes = 51):
        super(ResNet, self).__init__()
        self.inplanes = inplanes
        self.layer2 = self._make_layer(block, 512, 1, stride = 4)
...
class C1(nn.Module):
    def __init__(self, num_class=150, fc_dim=2048, use_softmax=False):
        ...
        self.cbr = conv3x3_bn_relu(fc_dim, fc_dim // 4, 1)
        # last conv
        self.conv_last = nn.Conv2d(fc_dim // 4, 1, 1, 1, 0)
        self.num_class=num_class
        # define a small resnet
        self.cls_net = ResNet(block = ResidualBlock, inplanes = fc_dim, num_classes=num_class)

    def forward(self, conv_out, segSize=None, res=None):
        conv5 = conv_out[-1]
        assert not torch.isnan(conv5).any(), "conv5 contains NaN values!"
        x = self.cbr(conv5)
        x = self.conv_last(x)
        ...
        assert not torch.isnan(cls_pred_new).any(), "cls_pred_new contains NaN values!"
        return cls_pred_new
