...
from DynamicFocus import preset
...
def train(segmentation_module, iterator, optimizers, epoch, cfg, history=None, foveation_module=None, writer=None, rank = None):
    ...
    tic = time.time()
    i = 0
    for X_Bx4xHSxWS, F_Bx2, Y_Bx1xHSxWS, Y_cls_Bx1 in iterator:
...
def create_optimizers(nets, cfg):
    ...
    if cfg.TRAIN.optim.lower() == 'sgd':
        ...
        optimizer_encoder = torch.optim.Adam(
            [{'params': net_encoder.parameters(),'lr_mult':cfg.TRAIN.lr_mult_encoder,'zoom':False}],    # cfg.TRAIN.lr_mult_encoder=0.001
            lr=cfg.TRAIN.lr_encoder,    # 2e-5
            weight_decay=cfg.TRAIN.weight_decay)    # 0.0001
        optimizer_decoder = torch.optim.Adam(
            [{'params': net_decoder.parameters(),'lr_mult':cfg.TRAIN.lr_mult_decoder,'zoom':False}],    # cfg.TRAIN.lr_mult_decoder=0.001
            lr=cfg.TRAIN.lr_encoder,
            weight_decay=cfg.TRAIN.weight_decay)
        optimizer_saliency = torch.optim.Adam(
            [{'params': net_saliency.parameters(),'lr_mult':cfg.TRAIN.lr_mult_saliency,'zoom':True}],   # cfg.TRAIN.lr_mult_saliency=0.001
            lr=cfg.TRAIN.lr_encoder,
            weight_decay=cfg.TRAIN.weight_decay)
        optimizer_compress = torch.optim.Adam(
            [{'params': net_compress.parameters(),'lr_mult':cfg.TRAIN.lr_mult_compress,'zoom':True}],   # cfg.TRAIN.lr_mult_compress=0.001
            lr=cfg.TRAIN.lr_encoder,
            weight_decay=cfg.TRAIN.weight_decay)
...
def main(rank, world_size, cfg, gpus):
    ...
    net_encoder = ModelBuilder.build_encoder(
        arch=cfg.MODEL.arch_encoder.lower(),            # hrnetv2_nodownsp
        fc_dim=cfg.MODEL.fc_dim,                        # 960
        weights=cfg.MODEL.weights_encoder,              # './ckpt/lvis_50cls/encoder_epoch_120.pth'         
        dilate_rate=cfg.DATASET.segm_downsampling_rate) # 1
    net_decoder = ModelBuilder.build_decoder(
        arch=cfg.MODEL.arch_decoder.lower(),            # c1
        fc_dim=cfg.MODEL.fc_dim,                        # 960
        num_class=cfg.DATASET.num_class,
        weights=cfg.MODEL.weights_decoder)              # './ckpt/lvis_50cls/decoder_epoch_120.pth'

    net_saliency = ModelBuilder.build_net_saliency(
        cfg=cfg,
        weights=cfg.MODEL.weights_net_saliency)         # './ckpt/lvis_50cls/saliency_epoch_120.pth'
    ...
    #调试用
    #segmentation_module = segmentation_module.to(f"cuda:{rank}")
    ...
    dataset_train = PreprocessDataset(data_path=preset.dpath_data_cook, marker=cfg.DATASET.dataset_marker_train, 
    ...
    dataloader_train = DataLoader(dataset_train, batch_size=20, sampler=train_sampler, num_workers = 4)

    dataset_valid = PreprocessDataset(data_path=preset.dpath_data_cook, marker=cfg.DATASET.dataset_marker_valid, 
                                      dataset_name='lvis',dataset_partition='valid')
    valid_sampler = DistributedSampler(dataset_valid, num_replicas=world_size, rank=rank, shuffle=True)
    dataloader_valid = DataLoader(dataset_valid, batch_size=10, sampler=valid_sampler, num_workers = 4)
        ...
        if (epoch+1) % cfg.TRAIN.eval_per_epoch == 0:
            if cfg.VAL.dice:
                ...
                if cfg.VAL.y_sampled_reverse:
                    ...
                    # enter here
                    val_loss, val_acc, val_acc_bin_fg, val_acc_cls_fbg, val_acc_bin_fbge, ious = eval_during_train_deform(cfg, writer=writer, count=epoch+1, epoch=epoch+1, datasetloader_valid = dataloader_valid, rank = rank)
...
if __name__ == '__main__':
    ...
    print(f'here!!!!!!!!!!!!!! {cfg.TRAIN.running_lr_encoder}')
