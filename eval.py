...
# This evaluate function is not used
...
def evaluate_1epoch(segmentation_module, iterator, cfg, epoch=1, rank = None, history=None, foveation_module=None, writer=None):
    ...
    for X_Bx4xHSxWS, F_Bx2, Y_Bx1xHSxWS, Y_cls_Bx1 in iterator:
        ...
        batch_data = {'img_data': X_Bx3xHSxWS.to(rank), 'seg_label': Y_Bx1xHSxWS.to(rank), 'focus_point': F_Bx2.to(rank), 'cls_label': Y_cls_Bx1.to(rank)}
...
def eval_during_train_deform(cfg, writer=None, gpu=0, count=None, epoch=None, datasetloader_valid=None, rank=None):
    ...
    #pdb.set_trace()
    loss, acc, acc_bin_fg, acc_cls_fbg, acc_bin_fbg, ious = evaluate_1epoch(segmentation_module, datasetloader_valid, cfg, epoch, rank)
...
def main(cfg, gpu):
    ...
    segmentation_module = SegmentationModule(net_encoder, net_decoder, cfg)
