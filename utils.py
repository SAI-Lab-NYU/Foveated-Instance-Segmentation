...
def accuracy(preds, label):
    ...
    acc_sum = (valid * (preds == label)).sum()
    valid_sum = valid.sum()
    pixel_sum_final = ((valid + valid1)> 0).sum()
    acc = float(acc_sum) / (pixel_sum_final + 1e-10)
    return acc, pixel_sum_final

# def accuracy(pred_all, label_all):   
#     pred_all = torch.tensor(pred_all).unsqueeze(0)
#     label_all = torch.tensor(label_all).unsqueeze(0)
